window.onload = function () {
    ...
    let orderByJersNumBtn = document.querySelector("#b_jers-num");
    let btnStateJers = 0;
    orderByJersNumBtn.addEventListener("click", function (e) {
        let orderedJers = [...players];
        let orderedByJers = ((a,b) => {
            if (a.id < b.id) {
                return -1;
            } else if (a.id > b.id) {
                return 1;
        ...
        if (btnStateJers == 0) {
            orderedJers.sort(orderedByJers);
            btnStateJers++;
            orderByJersNumBtn.innerHTML = "Jersey &#8595;";
        } else if (btnStateJers == 1) {
            orderedJers.sort(orderedByJers).reverse();
            btnStateJers++;
            orderByJersNumBtn.innerHTML = "Jersey &#8593;";
        } else {
            btnStateJers = 0;
